...
#if __MACOSX__
    if(argc >= 1) {
        if(strstr(argv[0],"FreeDroid.app/Contents/MacOS")) {
            return;
        }
    }
...
FindAllThemes(void)
{
    ...
    for(i = 0; i < MAX_THEMES; i++) {
        if(AllThemes.theme_name[i]) {
            free(AllThemes.theme_name[i]);
        }
    ...
    for(location = 0; location < 4; location++) {
        ...
        if(location == 2) {
            strcpy(dname, "../Resources");
        }

        if(location == 3) {
            strcpy(dname, SDL_GetBasePath());
        }
        ...
        while( (entry = readdir(dir)) != NULL ) {
            ...
            if( (fp = fopen(fpath, "r")) != NULL) { // config.theme is readable
                fclose(fp);
                ...
                if( i < AllThemes.num_themes ) { // found it in the list already
                    DebugPrintf(1, "Theme '%s' is already listed ... \n", tname);
                    continue;
                } else { // not yet listed --> we found a new theme!
                    DebugPrintf(1, "Found new graphics-theme: %s \n", tname);
            ...
            } else { // config.theme not readable
    ...
    DebugPrintf(1, "Game starts using theme: %s\n", GameConfig.Theme_Name);

...
EnterLift(void)
{
    ...
    while(!FirePressedR()) {
        if(UpPressedR() || WheelUpPressed()) {
            if(upLift != -1) {  /* gibt es noch einen Lift hoeher ? */
        ...
        if(DownPressedR() || WheelDownPressed()) {
            if(downLift != -1) { /* gibt es noch einen Lift tiefer ? */
...
EnterKonsole(void)
{
    ...
    while(!ReenterGame) {
        ...
        if(UpPressedR() || WheelUpPressed()) {
            ...
            if(show_cursor) {
                SDL_WarpMouseInWindow(ne_window, Cons_Menu_Rects[pos].x + Cons_Menu_Rects[pos].w / 2,
 ...
 *        to call FD_Flip() to display the result!
...
show_droid_portrait(SDL_Rect dst, int droid_type, float cycle_time, int flags)
{
    ...
    if(!background) { // first call
        background = SDL_CreateRGBSurface(0, dst.w, dst.h, screen_bpp, 0, 0, 0, 0);
        SDL_SetSurfaceBlendMode(background, SDL_BLENDMODE_NONE);
        SDL_SetSurfaceAlphaMod(background, 255);
    ...
    if( (droid_type != last_droid_type) || (droid_pics == NULL)) {     // we need to unpack the droid-pics into our local storage
        if(droid_pics) {
            SDL_FreeSurface(droid_pics);
        }
        droid_pics = IMG_Load_RW(packed_portraits[droid_type], 0);
        ...
        if(!IMG_isJPG(packed_portraits[droid_type])) {
            SDL_SetSurfaceBlendMode(droid_pics, SDL_BLENDMODE_BLEND);
            SDL_SetSurfaceAlphaMod(droid_pics, 255);
        } else {
            SDL_SetSurfaceBlendMode(droid_pics, SDL_BLENDMODE_NONE);
        }

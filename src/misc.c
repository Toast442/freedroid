...
find_file(char * fname, char * subdir, int use_theme, int critical)
{
    ...
    for(i = 0; i < 3; i++) {
        ...
        if(i == 2) {
            strcpy(File_Path, SDL_GetBasePath());  /* then the FD_DATADIR */
    ...
    if(!found) { // how critical is this file for the game:
        switch(critical) {
            case WARNONLY:
...
#if __MACOSX__
void _mac_log(const char * logbuffer);
#endif

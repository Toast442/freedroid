...
DrawLineBetweenTiles(float x1, float y1, float x2, float y2, int Color)
{
    ...
    if( (x1 == x2) && (y1 == y2) ) {
        return;                           // nothing is to be done here
    ...
    if(x1 == x2) { // infinite slope!! special case, that must be caught!
        ...
        for( i = 0; i < (y2 - y1) * Block_Rect.w; i++ ) {
            ...
            if( (pixx <= User_Rect.x) ||
               ...
               (pixy <= User_Rect.y) ||
               (pixy >= User_Rect.y + User_Rect.h - 1) ) {
                continue;
            }
...
MakeGridOnScreen(SDL_Rect * Grid_Rectangle)
{
    ...
    if( Grid_Rectangle == NULL ) {
        Grid_Rectangle = &User_Rect;
    }
...
SetCombatScaleTo(float scale)
{
    ...
    if(firstcall) {
        Copy_Rect(Block_Rect, origBlock);             // keep that as a backup
    }
    firstcall = FALSE;
    ...
    for( j = 0; j < NUM_COLORS; j++ ) {
        for( i = 0; i < NUM_MAP_BLOCKS; i++ ) {
            ...
            // and optimize
            MapBlockSurfacePointer[j][i] = tmp;
        }
    }
...
InitPictures(void)
{
    ...
    for(col = 0; col < DROID_PHASES; col++) {
        ...
        SDL_SetSurfaceBlendMode(InfluencerSurfacePointer[col], SDL_BLENDMODE_BLEND);
        SDL_SetSurfaceBlendMode(EnemySurfacePointer[col], SDL_BLENDMODE_BLEND);
        SDL_SetSurfaceAlphaMod(InfluencerSurfacePointer[col], 255);
        SDL_SetSurfaceAlphaMod(EnemySurfacePointer[col], 255);
    }

    //  SDL_SetSurfaceAlphaMod( Me.pic, SDL_ALPHA_OPAQUE);
    ...
    if(first_call) {
        ...
        BuildBlock = SDL_CreateRGBSurface(0, Block_Rect.w, Block_Rect.h, screen_bpp, 0xff, 0xff00, 0xff0000, 0xff000000);
        SDL_SetSurfaceBlendMode(BuildBlock, SDL_BLENDMODE_BLEND);
        SDL_SetSurfaceAlphaMod(BuildBlock, 255);
...
load_raw_pic(char * fpath)
{
    ...
    return (SDL_RWFromMem(mem, (int)size));
...
Load_Block(char * fpath, int line, int col, SDL_Rect * block, int flags)
{
    ...
    ret = SDL_CreateRGBSurface(0, dim.w, dim.h, screen_bpp, 0xff, 0xff00, 0xff0000, 0xFF000000);
    if(usealpha) {
    ...
    if(usealpha) {
        SDL_SetSurfaceAlphaMod(ret, 255);            /* clear per-surf alpha for internal blit */
...
Init_Video(void)
{
    ...
    screen_bpp = 32; /* start with the simplest */
    ...
    ne_window = SDL_CreateWindow("Freedroid",
                                 SDL_WINDOWPOS_UNDEFINED,
                                 SDL_WINDOWPOS_UNDEFINED,
                                 Screen_Rect.w * 1.5, Screen_Rect.h * 1.5,
                                 flags);

    ne_renderer = SDL_CreateRenderer(ne_window, -1, SDL_RENDERER_PRESENTVSYNC);

    SDL_SetHint(SDL_HINT_RENDER_SCALE_QUALITY, "linear");
    SDL_RenderSetLogicalSize(ne_renderer, Screen_Rect.w, Screen_Rect.h);
    ...
    ne_screen = SDL_CreateRGBSurface(0, Screen_Rect.w, Screen_Rect.h, 32,
                                     0x00FF0000,
                                     0x0000FF00,
                                     0x000000FF,
                                     0xFF000000);

    SDL_SetSurfaceBlendMode(ne_screen, SDL_BLENDMODE_NONE);



#if !__MACOSX__
    fpath = find_file(ICON_FILE, GRAPHICS_DIR, NO_THEME, WARNONLY);
    SDL_SetWindowIcon(ne_window, IMG_Load(fpath));
#endif
...
void
FD_Set_Gamma(float r, float g, float b)
{
    Uint16 redRamp[256];
    Uint16 greenRamp[256];
    Uint16 blueRamp[256];

    SDL_CalculateGammaRamp(r, redRamp);
    SDL_CalculateGammaRamp(g, greenRamp);
    SDL_CalculateGammaRamp(b, blueRamp);

    SDL_SetWindowGammaRamp(ne_window, redRamp, greenRamp, blueRamp);
}
...
white_noise(SDL_Surface * bitmap, SDL_Rect * rect, int timeout)
{
    ...
    SDL_BlitSurface(bitmap, rect, tmp, NULL);
    //  printf_SDL (ne_screen, rect->x + 10, rect->y + rect->h/2, "Preparing noise-tiles ");
    for(i = 0; i < NOISE_TILES; i++) {
        noise_tiles[i] = SDL_CreateRGBSurface(0, rect->w, rect->h, screen_bpp, 0, 0, 0, 0);
...
ScaleGraphics(float scale)
{
    ...
    if(first_call) {
        ...
        // get the Ashes pics
        if(Decal_pics[0]) {
            ScalePic(&Decal_pics[0], scale);
        }
        if(Decal_pics[1]) {
            ScalePic(&Decal_pics[1], scale);
        }

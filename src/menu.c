...
QuitGameMenu(void)
{
    ...
    while( (!KeyIsPressed('n')) && (!KeyIsPressed('y')) ) {
        SDL_Delay(1);
    }
    if(KeyIsPressed('y')) {
        Terminate(OK);
        ...
        while(!key) {
            ...
            if(UpPressedR() || WheelUpPressed() ) {
                ...
                if(MenuPosition > 1) {
                    ...
                    MenuPosition = POS_QUIT;
                }
                MoveMenuPositionSound();
            }
            if(DownPressedR() || WheelDownPressed() ) {
                key = TRUE;
                if( MenuPosition < POS_QUIT ) {
                    MenuPosition++;
                } else {
                    MenuPosition = 1;
                }
...
Options_Menu(void)
{
    ...
    while(!finished) {
        ...
        if(GameConfig.FullUserRect) {
            strcat(window_string, "Full");
        } else {
            strcat(window_string, "Classic");
        }
        ...
        while(!key) {
            ...
            switch(MenuPosition) {
                case SET_FULLSCREEN_FLAG:
                    if(FirePressedR()) {
                        SDL_SetWindowFullscreen(ne_window, SDL_WINDOW_FULLSCREEN_DESKTOP);
                        GameConfig.UseFullscreen = !GameConfig.UseFullscreen;
                        if(GameConfig.UseFullscreen) {
                            SDL_SetWindowFullscreen(ne_window, SDL_WINDOW_FULLSCREEN_DESKTOP);
                        } else {
                            SDL_SetWindowFullscreen(ne_window, 0);
                        }
                ...
                case SET_GAMMA_CORRECTION:
                    if(RightPressedR() || MouseRightPressedR()) {
                        ...
                        FD_Set_Gamma(GameConfig.Current_Gamma_Correction,
...
Show_Waypoints(void)
{
    ...
    for(wp = 0; wp < CurLevel->num_waypoints; wp++) {
        ...
        for(i = Block_Rect.w / 4; i < 3 * Block_Rect.w / 4; i++) {
            ...
            if( (x < User_Rect.x) || (x > User_Rect.x + User_Rect.w) || (y < User_Rect.y) || (y > User_Rect.y + User_Rect.h) ) {
                continue;
            }
...
LevelEditor(void)
{
    ...
    while( !Done ) {
        ...
        if(RightPressedR()) {
            if( rintf(Me.pos.x) < CurLevel->xlen - 1 ) {
                Me.pos.x += 1;
            }
        }
        ...
        if(DownPressedR()) {
            if( rintf(Me.pos.y) < CurLevel->ylen - 1 ) {
                Me.pos.y += 1;
            }
        }
        ...
        if( KeyIsPressedR(SDLK_F1) ) {
            ...
            FD_Flip(ne_screen);
            while(!FirePressedR() && !EscapePressedR()) {
                SDL_Delay(1);
            }
        }
        ...
        if( KeyIsPressedR('e') ) {
            ...
            if( SpecialMapValue >= NUM_MAP_BLOCKS ) {
                SpecialMapValue = 0;
            }
        ...
        if(KeyIsPressedR(SDLK_KP_0)) {
            CurLevel->map[BlockY][BlockX] = H_WALL;
        }
        if(KeyIsPressedR(SDLK_KP_1)) {
            CurLevel->map[BlockY][BlockX] = ECK_LU;
        }
        if(KeyIsPressedR(SDLK_KP_2)) {
            if(!ShiftPressed()) {
        ...
        if(KeyIsPressedR(SDLK_KP_3)) {
            CurLevel->map[BlockY][BlockX] = ECK_RU;
        }
        if(KeyIsPressedR(SDLK_KP_4)) {
            if(!ShiftPressed()) {
        ...
        if(KeyIsPressedR(SDLK_KP_5)) {
            if(!ShiftPressed()) {
                CurLevel->map[BlockY][BlockX] = KREUZ;
            } else {
                CurLevel->map[BlockY][BlockX] = VOID;
            }
        }
        if(KeyIsPressedR(SDLK_KP_6)) {
            if(!ShiftPressed()) {
        ...
        if(KeyIsPressedR(SDLK_KP_7)) {
            CurLevel->map[BlockY][BlockX] = ECK_LO;
        }
        if(KeyIsPressedR(SDLK_KP_8)) {
            if(!ShiftPressed()) {
        ...
        if(KeyIsPressedR('t')) {
            if(ShiftPressed()) {
                ...
                CurLevel->map[BlockY][BlockX] = H_ZUTUERE;
            }
        }
